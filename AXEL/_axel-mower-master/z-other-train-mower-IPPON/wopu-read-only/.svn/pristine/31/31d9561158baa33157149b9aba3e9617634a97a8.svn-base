/**
	SmartTextBox 1.00.23
	@author Pierre 'Spark' Gayvallet
	@site http://wayofspark.com/projects/smarttextbox
	@inspiredby Guillermo Rauch's TextBoxList plugin for mootools
...
SmartTextBox.SmartTextBox = SmartTextBox.BaseClass.extend({
	_options : {
		...
		autocompleteOptions : {  			// options
			minLength : 2,       			// min length to type to receive suggestions
	...
	construct : function(el, o){
		...
		if(this.options.autocomplete){
			this.autocomplete = new SmartTextBox.AutoComplete(this, this.autocompleteOptions);
			this.autocomplete.setValues(this.options.autocompleteValues);
		}
	...
	serialize : function(){
		...
		$.each(this.elements, function(i, el){
			...
			serialized += this.getValue() + self.options.separator;
		});
...
SmartTextBox.AutoComplete = SmartTextBox.BaseClass.extend({
	options : {
		...
		caseSensitive : false,
		highlight : true,
		placeholder : "Please start typing to receive suggestions"
	},
	...
	search : function(elem){		
		...
		if(value.length<this.options.minLength) this.showPlaceHolder();
	...
	searchResults : function(value){
		var newvals = [], regexp = new RegExp('\\b' + value, this.caseSensitive ? '' : 'i');
		var values = this.getValues(); 
		for (var i = 0; i < values.length; i++){
			if (newvals.length >= this.maxResults) break;
			if (regexp.test(values[i])) newvals.push(values[i]);
		}
		return newvals;
	},
	...
	addCurrent : function(){
		var value = this.currentSelection.html(); 
